...
const chartOutputHash = sha256(chartJson);

writeFileSync(
  resolve(generatedDir, "chart-data.js"),
  `/* Generated by npm run build. Input SHA-256: ${inputHash}. Output SHA-256: ${chartOutputHash} */\nwindow.COMPUTE_COST_DATA = ${chartJson};\n`
);
...
const versionedIndexHtml = indexHtml.replace(
  ...
  `generated/chart-data.js?v=${chartOutputHash.slice(0, 12)}`
);
if (versionedIndexHtml !== indexHtml) writeFileSync(indexPath, versionedIndexHtml);

const summary = {
  metadata: outputMetadata,
  chartDataSha256: chartOutputHash,
...
const regionalCoverageHeaders = [
  "input_sha256", "layer", "layer_label", "country", "region", "covered_capacity_mw",
  "registered_capacity_mw", "regional_coverage_pct", "covered_source_records",
...
writeFileSync(resolve(ROOT, "regional-coverage-breakdown.csv"), toCsv(regionalCoverageHeaders,
  coverage.regionBreakdowns.flatMap((layer) => layer.regions.map((region) => ({
    ...
    covered_capacity_mw: round(region.capacityMw, 4),
    registered_capacity_mw: round(region.registeredCapacityMw, 4),
    regional_coverage_pct: round(region.coveragePct, 4),
    covered_source_records: region.recordCount,
    registered_source_records: region.registeredRecordCount
  })))
));
...
- Regional percentages divide each layer's covered MW by that same region's registered MW. All **${coverage.capacityRegister.registeredRegionCount}** registered regions remain in every breakdown, including explicit 0% price-coverage rows; the figures are not shares of the layer total.
